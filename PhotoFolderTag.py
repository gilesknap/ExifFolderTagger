...
root_folder = os.path.abspath("E:/PhotosWithTagsTmp")
do_write = True
do_exif = True
tag_name2 = "Exif.Image.XPTitle"
...
# TODO restore above
renamed_exp = re.compile('XY\d\d\d\d.*')
        ...
        for filename in fileList:
            ...
            if ext.lower() == '.jpg':
                ...
                if do_exif:
                    ...
                    if tag_name in metadata.exif_keys:
                        tag = metadata[tag_name]
                        meta_desc = str(tag.raw_value).strip()
                        ...
                        if renamed_exp.match(meta_desc):
                            ...
                            if not (len(meta_desc) == 0 or any(s in meta_desc for s in descriptions_to_ignore)):
                                new_meta_desc = description + '\n' + meta_desc
                            print TermColors.FAIL + "%s : >%s< --> >%s<" % \
                                                        (fullname, meta_desc, new_meta_desc) + TermColors.END_C
                    if do_write:
                        metadata[tag_name] = new_meta_desc
                        #metadata[tag_name2] = bytearray(unicode(new_meta_desc).encode("utf-8"))
                        metadata[tag_name2] = list(bytearray(unicode("Laughing"),'utf16'))[2:]
                        # metadata[tag_name2] = [1,2]
                        # metadata[tag_name2] = ''.join(chr(x) for x in [0x13, 0x00, 0x00, 0x00, 0x08, 0x00])
                        metadata.write()
            else:
